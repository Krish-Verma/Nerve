...
# ---------------------------------------------------------------------------------------------
#
# Eight repositories, one per way history can be read or misread:
#
...
#   history-similar    (Slice 12c-ii) thirteen similar-content rename candidates with hand-written
#                      ground truth, including the licence-header pair that sets the threshold and
#                      the large-edit rename the threshold misses.
#
...
def main():
    ...
    checks = {
        ...
        # The *candidate* rule only — a deletion paired with an addition whose blob oid differs.
        # Deliberately not a similarity measurement: computing one here would put a second
        # implementation of the matcher inside the fixture that grades it, and the expected values
        # live in the hand-written `ground_truth.json` instead.
        "similarity_candidates": lambda: any(
            removed["prev_blob_oid"] != appeared["blob_oid"]
            for commit in commits
            for removed in (commit["changes"] or [])
            if removed["change_kind"] == "deleted"
            for appeared in (commit["changes"] or [])
            if appeared["change_kind"] == "added"
        ),
...
# The same header for the one fixture that belongs to a later slice.
#
# A separate function rather than a parameter on `readme_head`, because editing `readme_head` would
# rewrite all seven sibling READMEs — and this script's determinism claim is that regenerating
# changes nothing else. Two lines duplicated is cheaper than seven files of churn.
readme_head_similar() { # readme_head_similar <fixture> <title>
  cat <<EOF
# \`fixtures/$1\` — $2 (Slice 12c-ii)
...
# ---------------------------------------------------------------------------------------------
# 8. history-similar  (Slice 12c-ii)
# ---------------------------------------------------------------------------------------------
#
# The corpus the similar-content matcher's precision is measured on. Its ground truth is
# `fixtures/history-similar/ground_truth.json`, which is **hand-written** and is not produced here:
# a fixture whose expected answers were generated by the thing they grade would make the precision
# test vacuous. This section builds the repository; that file says what the answer is.
#
# Every file is twenty numbered lines unless a case needs otherwise, and every line is distinct
# within its file, so the multiset intersection is a count of shared line strings that a reviewer
# can do by hand. `fixtures/history-similar/README.md` states the arithmetic per commit.

lines() { # lines <prefix> <first> <count> — <count> distinct lines, numbered from <first>
  local prefix="$1" first="$2" count="$3" i
  for ((i = first; i < first + count; i++)); do
    printf '%s line %02d\n' "$prefix" "$i"
  done
}

rlines() { # rlines <prefix> <first> <count> — the same lines, last one first
  local prefix="$1" first="$2" count="$3" i
  for ((i = first + count - 1; i >= first; i--)); do
    printf '%s line %02d\n' "$prefix" "$i"
  done
}
...
new_repo similar
cd "$work/similar"
mkdir -p mod conf legal fan ord tiny bin copy gen empty

# c0 creates every file that is later deleted, so each scenario below is one commit with one
# deletion group and one addition group and no interference between scenarios.
lines alpha 1 20 >mod/alpha.txt
lines beta 1 20 >mod/beta.txt
lines gamma 1 20 >mod/gamma.txt
{ lines scaffold 1 14; lines old-service 15 6; } >conf/old-service.yaml
{ lines licence 1 16; lines old-body 17 4; } >legal/old.txt
lines one 1 20 >fan/one.txt
{ lines merged 1 18; lines left 19 2; } >fan/left.txt
{ lines merged 3 18; lines right 1 2; } >fan/right.txt
lines ord 1 20 >ord/source.txt
lines tiny 1 2 >tiny/small.txt
# Written by python3 rather than printf: a literal NUL cannot survive a shell variable, and the
# whole point of these two blobs is that they contain one.
python3 -c 'open("bin/blob.bin","wb").write(b"NERVE\x00binary payload one\x00\x01\x02\n")'
lines copy 1 20 >copy/original.txt
{ lines lock 1 10; lines old-lock 11 10; } >gen/old.lock
: >empty/a.txt
at 0
git add -A
git commit -q -m "root commit: every file that is later deleted"
similar0="$(git rev-parse HEAD)"

# c1 — a true rename with a small edit. 18 of 20 lines survive: 18/20, admitted at 7/8.
at 1
{ lines alpha 1 18; lines alpha-new 19 2; } >mod/alpha-renamed.txt
git rm -q mod/alpha.txt
git add mod/alpha-renamed.txt
git commit -q -m "c1: move mod/alpha.txt to mod/alpha-renamed.txt with two lines edited"
similar1="$(git rev-parse HEAD)"

# c2 — a true rename with a large edit. 8 of 20 lines survive: 8/20, rejected. A **false
# negative**, and the fixture exists to publish it rather than to hide it.
at 2
{ lines beta 1 8; lines beta-edit 9 12; } >mod/beta-moved.txt
git rm -q mod/beta.txt
git add mod/beta-moved.txt
git commit -q -m "c2: move mod/beta.txt to mod/beta-moved.txt with twelve lines edited"
similar2="$(git rev-parse HEAD)"

# c3 — a deletion and an unrelated addition. 0/20.
at 3
lines unrelated 1 20 >mod/unrelated.txt
git rm -q mod/gamma.txt
git add mod/unrelated.txt
git commit -q -m "c3: delete mod/gamma.txt and add an unrelated mod/unrelated.txt"
similar3="$(git rev-parse HEAD)"

# c4 — shared boilerplate, not a move. 14 scaffold lines of 20: 14/20, rejected.
at 4
{ lines scaffold 1 14; lines new-thing 15 6; } >conf/new-thing.yaml
git rm -q conf/old-service.yaml
git add conf/new-thing.yaml
git commit -q -m "c4: two YAML files sharing fourteen scaffold lines and nothing else"
similar4="$(git rev-parse HEAD)"

# c5 — **the pair that sets the threshold.** A sixteen-line licence header of 20: 16/20 = 4/5, so
# any threshold at or below 4/5 admits it and the false-positive count stops being zero.
at 5
{ lines licence 1 16; lines new-body 17 4; } >legal/new.txt
git rm -q legal/old.txt
git add legal/new.txt
git commit -q -m "c5: two files sharing a sixteen-line licence header and nothing else"
similar5="$(git rev-parse HEAD)"

# c6 — one to many. Both pairings measure 18/20 and both are admitted, `many_to`.
at 6
{ lines one 1 18; lines one-a 19 2; } >fan/one-a.txt
{ lines one 1 18; lines one-b 19 2; } >fan/one-b.txt
git rm -q fan/one.txt
git add fan/one-a.txt fan/one-b.txt
git commit -q -m "c6: delete fan/one.txt and add two files that each keep eighteen of its lines"
similar6="$(git rev-parse HEAD)"

# c7 — many to one. merged 01-18 is fan/left.txt's first eighteen lines and merged 03-20 is
# fan/right.txt's first eighteen, so both pairings measure 18/20, `many_from`.
at 7
lines merged 1 20 >fan/merged.txt
git rm -q fan/left.txt fan/right.txt
git add fan/merged.txt
git commit -q -m "c7: delete fan/left.txt and fan/right.txt and add fan/merged.txt"
similar7="$(git rev-parse HEAD)"

# c8 — the same twenty lines, reversed. 20/20, because a multiset cannot see order. Documented
# rather than patched, and the two blob oids really do differ.
at 8
rlines ord 1 20 >ord/shuffled.txt
git rm -q ord/source.txt
git add ord/shuffled.txt
git commit -q -m "c8: move ord/source.txt to ord/shuffled.txt with the same lines reordered"
similar8="$(git rev-parse HEAD)"

# c9 — two lines each, beneath the three-line floor. Unmeasurable, `blob-too-small`.
at 9
{ lines tiny 1 1; lines tiny-b 2 1; } >tiny/small2.txt
git rm -q tiny/small.txt
git add tiny/small2.txt
git commit -q -m "c9: move tiny/small.txt to tiny/small2.txt, two lines each"
similar9="$(git rev-parse HEAD)"

# c10 — both blobs contain NUL. Unmeasurable, `blob-binary`.
at 10
python3 -c 'open("bin/other.bin","wb").write(b"NERVE\x00binary payload two\x00\x03\x04\n")'
git rm -q bin/blob.bin
git add bin/other.bin
git commit -q -m "c10: move bin/blob.bin to bin/other.bin, both containing NUL"
similar10="$(git rev-parse HEAD)"

# c11 — a copy, not a move: the source stays in the tree, so there is no deletion and therefore no
# candidate. The 19 shared lines are never measured, which is the point.
at 11
{ lines copy 1 19; lines copy-extra 20 1; } >copy/duplicate.txt
git add copy/duplicate.txt
git commit -q -m "c11: copy copy/original.txt to copy/duplicate.txt, leaving the original in place"
similar11="$(git rev-parse HEAD)"

# c12 — two generated lockfiles sharing a ten-line preamble. 10/20, rejected.
at 12
{ lines lock 1 10; lines new-lock 11 10; } >gen/new.lock
git rm -q gen/old.lock
git add gen/new.lock
git commit -q -m "c12: replace gen/old.lock with gen/new.lock, sharing ten lines"
similar12="$(git rev-parse HEAD)"

# c13 — two empty files are one blob oid, so this is the **exact** matcher's row and the similarity
# matcher must skip it. Emitting it under both evidence values would be the blend §6 forbids.
at 13
: >empty/b.txt
git rm -q empty/a.txt
git add empty/b.txt
git commit -q -m "c13: move empty/a.txt to empty/b.txt, both empty"
similar13="$(git rev-parse HEAD)"

# `emit_gitdir` deletes the fixture directory whole, which would take the hand-written
# `ground_truth.json` with it. It is carried across the rebuild rather than rewritten, because
# rewriting it here is precisely what it must never be: the oracle cannot be produced by the thing
# it grades. A missing one is a hard failure rather than a silent regeneration.
similar_truth="$fixtures/history-similar/ground_truth.json"
if [ ! -f "$similar_truth" ]; then
  echo "fixtures/history-similar/ground_truth.json is missing." >&2
  echo "It is HAND-WRITTEN and this script does not create it: a fixture whose expected answers" >&2
  echo "were generated by the matcher they grade would make the precision test vacuous." >&2
  echo "Restore it from git before regenerating this fixture." >&2
  exit 1
fi
cp "$similar_truth" "$work/ground_truth.json"
emit_gitdir "$work/similar/.git" "$fixtures/history-similar" "$similar13"
cp "$work/ground_truth.json" "$similar_truth"
python3 "$work/inventory.py" --name history-similar \
  --gitdir "$fixtures/history-similar/gitdir" \
  --out "$fixtures/history-similar/inventory.json" \
  --commit "$similar0" --commit "$similar1" --commit "$similar2" --commit "$similar3" \
  --commit "$similar4" --commit "$similar5" --commit "$similar6" --commit "$similar7" \
  --commit "$similar8" --commit "$similar9" --commit "$similar10" --commit "$similar11" \
  --commit "$similar12" --commit "$similar13" \
  --require root_commit --require added --require deleted \
  --require unique_rename --require similarity_candidates

{
  readme_head_similar history-similar "the similar-content matcher's measured corpus"
  cat <<'EOF'
## What this exercises
...
EOF
} >"$fixtures/history-similar/README.md"
...
for name in history-basic history-shallow history-rename history-merge history-worktree \
  history-missing history-hostile history-similar; do
