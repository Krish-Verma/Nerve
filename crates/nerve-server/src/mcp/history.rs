...
pub fn descriptor() -> Value {
    json!({
        ...
        "description": concat!(
            ...
            "A commit summary is bounded, and every commit carries `summary_truncation` beside it ",
            "— `complete`, `truncated` or `unknown`. Read it before you quote a summary: the text ",
            "alone cannot tell a short first line from a cut one, a first line of exactly the ",
            "bound is `complete`, and `unknown` means the commit was recorded before Nerve stored ",
            "the answer, so it cannot be recovered rather than being a quiet `complete`.\n\n",
            "A rename is a hypothesis and never a rename Git recorded, because Git records none. ",
            "Each carries `evidence` — `exact_content`, where both paths name one blob, or ",
            "`similar_content`, where a named method measured how much two different blobs share. ",
            "A similarity hypothesis carries `matcher_id`, `matcher_version`, the measurement as ",
            "`match_numerator` of `match_denominator` — two integers, never a percentage and never ",
            "a float — and an `analysis` object holding the `threshold_numerator` of ",
            "`threshold_denominator` it was admitted against and the `completeness` of the ",
            "commit's candidate set. The two evidence values are separate claims and are never ",
            "blended, summed or ranked against each other: an exact match carries no measurement ",
            "at all, which is not a perfect score. Where `analysis` is null, ",
            "`analysis_absent_note` says why.\n\n",
