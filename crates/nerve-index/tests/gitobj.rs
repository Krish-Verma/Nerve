...
///
/// **The literal moved from 5 to 6 in Slice 12b, from 6 to 7 in Slice 12c-ii and from 7 to 8 in
/// Slice 13a-i, and it is still a tripwire rather than a bookkeeping value.** 12a was a reader with
/// no schema change of its own, which is what this test was written to pin. v6 belongs to 12b, v7
/// to 12c-ii and v8 to 13a-i, and all three live entirely in `nerve-store`; the invariant that
/// survives is that the *reader* is never what moves the version, and what enforces that is the
/// source scan below — no module under `crates/nerve-index/src/gitobj` may name an entity kind, a
/// relation, or the evidence tables. Pinning the number keeps the next slice that touches the
