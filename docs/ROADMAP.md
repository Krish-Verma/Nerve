...
| 11b | Reference Python tracer — `tracers/python/nerve_trace/`, `sys.monitoring` + `sys.settrace`, real pytest end to end | 🔨 In progress — plan committed (`docs/plans/slice-11b-python-tracer.md`): a **pytest plugin** rather than a bare tracer, because `sys.monitoring` reports code objects and cannot know which test is running, which is why the contract carries `test_framework`; inert without its flag; refuses to install when `sys.gettrace()` is occupied rather than displacing `coverage.py`; "no argument or return value is **capturable**" as three layers, because any one alone is a promise rather than a property |
| 12a | Git **object access** — loose objects, `.idx` v2, pack entries, `OFS_DELTA`/`REF_DELTA`, alternates, shallow/partial detection | ⬜ Not started — dependency analysis committed (`docs/plans/slice-12-git-object-access-analysis.md`): a decompressor is **mandatory**, measured `78 01` on this repository's loose objects and there is no compression library among the 101 packages; **packed objects are mandatory too**, and Nerve's own `.git` (1342 loose, **0 packfiles**) is misleading because a clone is always packed — loose-only support would pass every fixture and find no history in a user's repository; `gix` rejected on a measured ~50% dependency-tree increase plus transport code an offline-first product should not carry, `git2` rejected as a C library with its own HTTP transport, `git cat-file` refused by `no_subprocess.rs`, and `flate2`/`rust_backend` recommended |
