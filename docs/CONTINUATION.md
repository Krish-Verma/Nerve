...
| **Remote** | **`origin` → `https://github.com/Krish-Verma/Nerve.git`, PUBLIC, `main` pushed and tracking.** Added 2026-08-10 **on direct user instruction**, which supersedes the standing local-only decision recorded below and the session brief's own "do not add a remote" rule — direct user instruction outranks both, and the superseded entry is kept so the change is visible rather than silent. The repository was **empty** before this. A safety scan ran first over the whole history: the only `.env` ever tracked is `fixtures/ts-basic/.env`, whose content is `EXAMPLE_TOKEN=not-a-real-secret-fixture-value` and which exists **to test the secret deny-list**; no absolute home path appears in any tracked file; `.nerve/`, `target/`, `node_modules/` and `apps/nerve-web/dist/` are gitignored. **Push each slice as it lands.** |
| **Superseded remote entry** | **None configured.** Nothing pushed. All work local. Deliberate — see "Decisions already made". |
| **Last completed slice** | **Row 13 complete** — 13a-i `2509511` (schema v8), 13a-ii `4965a76` (registry CLI + T12), 13b `b2afaba` (C1+C3), 13c `f336d5c` (C2), 13d `672eaa1` (HTTP+MCP+UI), 13d-ii (CLI link freshness). **Rows 1–13 complete.** |
| **Verification at row 13's close** | **1652+** Rust tests (0 failed, 2 ignored) · frontend **34** · acceptance **74+** · `Cargo.lock` 106 · `SCHEMA_VERSION` **8** · `EvidenceSourceType::ALL` **12**. |
| **Row 13's three precision tables — never summed** | C1 `npm_local_dependency` 5 TP · 0 FP · 1 FN · recall 0.833 · C3 `python_path_dependency` 5 TP · 0 FP · 1 FN · recall 0.833 · C2 `npm_export_resolution` 8 TP · 0 FP · 1 FN · recall 0.889. Each FN is a **declined true link** — a form outside the enumerated syntax that really does name a registered neighbour — counted and published rather than bought back by widening the syntax. |
| **Row 13's standing refusals** | A cross-repository link is **never** a local assertion (`assertion.target_entity_id` is a hard FK into the local `entity` table), and `neither_path_nor_impact_traverses_a_contract_link` asserts it negatively with anti-vacuity floors. **`contract_version_mismatch` has no producer** — `^1.2.0` against `1.2.3` is range satisfaction, which needs a semver resolver, i.e. a new dependency; both versions are recorded and no verdict is derived, and §9.3 requires **eleven** of twelve. No new `EvidenceSourceType`. No auto-discovery. No network fetch. |
| **SCHEMA IS NOW v8** | `repo_registry` (tombstoned, never deleted) and `contract_link`. **`contract_link`'s target columns are deliberately NOT foreign keys** — that is the whole correction: `assertion.target_entity_id` IS one (`schema.rs:97`) and a cross-repository target has no local `entity` row. A test proves both halves. **No new `EvidenceSourceType`** — `observation.assertion_id` is `NOT NULL`, so nothing could carry it; `ALL` is still 12. `ContractFreshness` has **twelve** states; `generated_client_stale` does not exist. V1–V7 byte-identical. Row 14 is therefore **v9**. |
| **Known incomplete in 13a-i, by design** | `relocate_registry_entry` moves a path **without** verifying the new path holds the expected `repo_id` (§6 requires it) — its doc comment names 13a-ii as owner. None of the twelve freshness states is produced from a fixture yet. |
...
| **Next action (2026-08-11, CURRENT)** | **The functional UI parity phase — ROW 14 IS CLOSED** (`e0254b0`). Verified at `e0254b0`: **1761 Rust tests**, frontend **48**, acceptance **88**, `Cargo.lock` 106, schema v10, `ROUTES` 23, `TOOL_NAMES` 8, `GLOSS_TABLES` 33 and **`DECLARED_NOT_RENDERED` empty**. Four parity gaps remain, re-derived from source rather than trusted: **(1)** `/api/impact` has existed since 7b and the string `impact` appears **nowhere** under `apps/nerve-web/src/`; **(2)** selector alternatives are never rendered, which is 8b-i's whole point unsurfaced; **(3)** trace / `TEST_OBSERVED_CALL` evidence has no dedicated surface — **and there is no `/api/trace` route**, so decide first whether one is needed or whether it already reaches the UI through `/api/why` and `/api/entity`; **(4)** `nerve check`'s trust verdict is CLI-only and there is **no `/api/check` route**, so that gap is a full vertical slice (HTTP + MCP + UI), not a UI-only one. Gaps 1–3 are in flight. **Keep `DECLARED_NOT_RENDERED` empty** — surfacing a vocabulary without its gloss re-opens the drift 5d-iii was a corrective slice for. Browser QA: `scripts/viewport_qa.mjs` over CDP at 1600px and 380px; **`--window-size` is a trap** (Chrome clamps to ~500 CSS px and crops the PNG). After parity: the canonical vocabulary mechanism, real-world validation (`docs/plans/slice-15-real-world-validation.md`), semantic acceptance, and the clean-checkout audit. |
| **Superseded next action (2026-08-11)** | **Slice 14d — the functional reference UI, which CLOSES ROW 14.** 14a/14b/14c are complete: the store, the ten-verb CLI, and the read-only HTTP + MCP surfaces all exist. Verified at `6a56ee7`: **1758 tests**, acceptance **87**, `ROUTES` 23, `TOOL_NAMES` 8, `Cargo.lock` 106, schema v10. 14d consumes `GET /api/memory` and `GET /api/memory/record?memory_id=` (note: **not** a path parameter — the router matches an exact table and a parity test scrapes string-literal arms in both directions). It must render stored lifecycle and derived views **distinguishably** (they are different kinds and §3 spent its length on that), all three `result_kind` values (`no_memory_recorded` and `no_memory_matches` are different absences), and for every CLI-only mutation must explain the read-only boundary and print the **exact copyable command** from `boundary.commands` — never a fake disabled button. **Five vocabularies need glosses and the `ui_vocabulary.rs` guard extension in the same commit** (`MemoryStatus` 4, `MemoryView` 3, `MemorySubjectResolution` 5, `MemoryScope` 4, `MemoryOperation` 5) — 5d-iii is the precedent: a guard that does not know a vocabulary exists cannot fail for it, and 12c-iv found eight in that state. Browser QA at 1600px and **380px** with `scripts/viewport_qa.mjs` (CDP `Emulation.setDeviceMetricsOverride`); **`--window-size=380` is a trap** that has bitten this project twice — Chrome clamps to ~500 CSS px and merely crops the PNG. After 14d: the four UI parity gaps, the canonical vocabulary mechanism, real-world validation, semantic acceptance, and the clean-checkout audit. |
| **Superseded next action (2026-08-11)** | **Slice 14c — read-only HTTP + MCP + T7 + T13.** **Row 14b is COMPLETE**: 14b-i `dd55152` (schema v10, closed vocabularies, transactional lifecycle writes) and 14b-ii `bdc87fa` (the ten-verb `nerve memory` command family, citations, deterministic export, and the acceptance rows that replaced the existence-only loop). Verified at `bdc87fa`: **1730 tests**, acceptance **83**, `Cargo.lock` 106, `SCHEMA_VERSION` 10. 14c adds `GET /api/memory` and `GET /api/memory/<id>` (no `POST`/`PUT`/`PATCH`/`DELETE` — 405, database byte-identical), the MCP read surface, the T7 extension for memory text, and a **source scan** proving no lifecycle write is reachable from `crates/nerve-server/src/mcp/`. **T13 must be written by the orchestrator into `docs/THREAT-MODEL.md` and `docs/SECURITY.md`** — T12 is currently the last entry, so T13 is unclaimed: *Nerve cannot distinguish a human from an agent when both control the same shell.* Then **14d** the UI, which closes row 14. |
| **Superseded next action (2026-08-10)** | **Slice 14b-ii — the `nerve memory` command family.** 14b-i (`dd55152`) landed everything below it: schema **v10** closes `memory.scope` (four values — `implementation`, `interface`, `operations`, `process`) and `memory_event.operation` (five — `proposed`, `confirmed`, `superseded`, `invalidated`, `cited`) in SQL, and `crates/nerve-store/src/memory.rs` now has the **transactional** `propose_memory`, `confirm_memory`, `invalidate_memory`, `cite_memory` beside the existing `supersede_memory` (whose `operation` is now a `MemoryOperation`, not a `&str`). **Nothing outside `nerve-core` and `nerve-store` has been touched — there is no CLI, HTTP, MCP or UI surface for memory at all.** Build `nerve memory <propose\|confirm\|list\|show\|search\|supersede\|invalidate\|cite\|events\|export>` in `crates/nerve-cli/src/main.rs` (6075 lines), following **`nerve repo`** — it is the newest and closest analogue: `RepoCommand` at `:574`, dispatch at `:854-870`, `open_registry` for the `(OpenIndex, repo_id)` pair, `run_repo_list` at `:4395` for the shape of a handler, `resolve_one` at `:5071` for selector resolution, and `Output::{line,object,failure,failure_detail}`. **Five things that are not optional.** (1) **There is no `delete` verb** — a delete verb is how "history preserved" stops being true. (2) **`scripts/final_acceptance.sh`'s existence-only loop (search for `for pair in "memory:Slice 14`) must be replaced in the same commit that lands the command** — it prints `NOT BUILT` today and would print `PASS — nerve memory exists — update this script` the moment the command appears, which is the empty pass `2dc3a7d` had to delete for `history`. Replace with behaviour: propose persists · confirm changes lifecycle · supersede preserves every prior event · invalidate stays distinct from supersede · a read mutates nothing · export contains the record just written. (3) **A subject snapshot needs the current `state_id` for `anchor_state_id`**, and `memory.rs`'s `current_state_id` is **private** — expose it (or an equivalent) rather than re-deriving it in the CLI, because it reads off the latest `extractor_run` and a second derivation is the duplication 12c-i-a existed to remove. (4) **Export ships** — versioned, deterministic, canonical key order, byte-identical twice. (5) **Import is NOT in 14b's scope** per plan §6c; it ships only under the full §6b safety contract or is deferred explicitly. Then **14c** read-only HTTP + MCP + T7 + **T13** (T12 is the last threat-model entry, so T13 is unclaimed), **14d** UI + the semantic acceptance package. |
| **Superseded next action (2026-08-10)** | **Slice 14b — the CLI lifecycle.** Everything it needs exists: schema v9 is applied and `crates/nerve-store/src/memory.rs` already has `insert_memory`, `insert_memory_citation`, `append_memory_event`, `supersede_memory` (one transaction), `memory`, `list_memory`, `memory_for_subject`, `memory_in_scope`, `memory_citations`, `memory_events`, `superseded_by`, `resolve_memory_subject`, `read_memory`, `read_memory_all`, `read_memory_for_subject`, `read_memory_in_scope`. **Build `nerve memory <note\|confirm\|supersede\|invalidate\|list\|show\|cite\|events\|export>`** in `crates/nerve-cli/src/main.rs` (6075 lines; follow `nerve repo`'s shape — it is the newest and closest analogue, `RepoCommand` + per-verb handlers + `--json`). **Four things that are not optional.** (1) **There is no `delete` verb** — a delete verb is how "history preserved" stops being true. ~~the CLI-surface test that holds `affected` and `trace-tests` refused is the mechanism that keeps it absent.~~ **FALSE, found by 14b-ii and corrected 2026-08-11: that test does not exist.** `affected` appears in no test under `crates/nerve-cli/tests/`, and `trace-tests` only in doc comments and the acceptance script, so those two refusals are held **only** by `final_acceptance.sh`'s `refused` helper — a script a developer may not run, not a gate. 14b-ii built a real `there_is_no_delete_verb` test for its own verb instead of inheriting the assumption. **Carried defect: `nerve affected` and `nerve trace-tests` still deserve the test this claim assumed they had.** (2) **`scripts/final_acceptance.sh:133`'s existence-only loop must be replaced in the same commit that lands the command** — it currently prints `NOT BUILT` (honest), and the moment `nerve memory` exists it prints `PASS — nerve memory exists — update this script`, which is the empty pass `2dc3a7d` had to delete for `history`. Replace with behaviour: creation persists · supersession preserves every prior event · invalidation is visible in a read · a read mutates nothing · the four stored statuses round-trip. (3) **`scope` has no defined domain** — 14a stored it as an opaque non-empty string and 14b decides the values (plan §14a correction 1). (4) **Export ships; import only if dry-run, schema validation, stable ids, duplicate handling, repository-mismatch refusal, bounds and transactional rollback can all be met** — otherwise defer import explicitly (plan §6b). Then **14c** read-only HTTP + MCP + T7 + T13, **14d** UI + the semantic acceptance package. |
| **Superseded next action (2026-08-10)** | **Row 14a — human-confirmed memory, schema v9.** **Read `docs/plans/slice-14-human-confirmed-memory.md`'s "Corrected 2026-08-08" blocks first; they overrule the surrounding text.** The one that decides the schema: **`memory.subject_entity_id` must NOT be a foreign key into `entity`.** Entity rows are deleted routinely — `prune_orphans` (`prune.rs:376`, `:440`), pinned as required behaviour by `deleting_a_file_removes_its_entities_assertions_and_observations` (`incremental.rs:290`) — so with `foreign_keys=ON` the FK either **blocks re-indexing a file a human wrote a note about**, or **cascade-destroys the note on a routine re-index**. Store subject snapshots and resolve at query time. Also corrected: MCP and HTTP are **read-only without exception** (persisting a `proposed` row from MCP would break the byte-identical test that already passes); the stored lifecycle is **four** values with **three** derived views (`potentially_stale`, `conflicted`, `multiple_active`); `conflicted` requires a shared optional `claim_key` or an explicit human conflict link, because free prose can never be ordered and the rule as drafted would call every second note a contradiction; supersession stores **one** direction; and export ships while import is conditional. Sub-slices 14a–14d are in the plan's §6c. |
| **Superseded next action (2026-08-08)** | **Row 13a — the local registry and schema v8.** **Read `docs/plans/slice-13-cross-repository-contracts.md`'s "Corrected 2026-08-08" blocks first; they overrule the surrounding text.** The correction that matters: **C2 cannot be a local assertion.** `assertion.target_entity_id` is `NOT NULL REFERENCES entity(entity_id)` (`schema.rs:97`, immutable since v1) with `foreign_keys=ON` (`db.rs:37`), and C2's target is a file in repository B with no row in A's `entity` table — so an ordinary local `REFERENCES` assertion **cannot be inserted**, and the only ways to force it (a proxy entity, or dropping the FK) are refused. Every cross-repository link lives in `contract_link`. The new `EvidenceSourceType` is **withdrawn** with it, because `observation.assertion_id` is `NOT NULL` so no observation could carry it; C2 uses a contract-specific closed vocabulary in `contract_link.resolution_method` instead. Also corrected: `contract_link` now carries **target snapshots** and a `withdrawn_at`/`status` lifecycle (without them a renamed target makes `contract_deleted`, `target_changed` and `contract_file_missing` indistinguishable), expected vs observed contract version, the freshness set is **twelve not thirteen** with `generated_client_stale` removed as unreachable (§2.1 of the same plan refuses the evidence it rests on), and `nerve repo remove` **tombstones** rather than deletes so `registry_entry_removed` stays reportable. Row 14's plan was corrected the same day (`d9120c8`) — most importantly `memory.subject_entity_id` must **not** be an FK into `entity`, because `prune_orphans` deletes entity rows routinely (`prune.rs:376`, `:440`) and the FK would either block re-indexing or cascade-destroy a human's note. |
...
| **Roadmap status** | **INCOMPLETE — and this is the authoritative statement of it.** **Rows 1–14 are COMPLETE.** Verified at `e0254b0`: `cargo fmt` clean · `clippy` 0 warnings · `cargo test --workspace --no-fail-fast` **1761 passed, 0 failed, 2 ignored, 60 targets** · frontend **48 passed** · `cargo build --release` Finished · `scripts/final_acceptance.sh` **88 passed, 0 failed, 0 skipped** · `ROUTES` **23** · `TOOL_NAMES` **8** · `GLOSS_TABLES` **33** · `DECLARED_NOT_RENDERED` **empty** · **T13 written** · `Cargo.lock` **106** · `SCHEMA_VERSION` **10** · `EvidenceSourceType::ALL` **12**. **The existence-only acceptance row is gone** — deleted in the same commit that landed the command, which is what row 14's plan §6 demanded and what `2dc3a7d` had to do for `history`. Not started: **the four UI parity gaps** · the four remaining functional-UI-parity gaps · the **real-world validation run** (`docs/plans/slice-15-real-world-validation.md`) · the **semantic acceptance package** (`final_acceptance.sh` still contains one existence-only row, `memory`, at `:133`) · the **clean-checkout audit**. The acceptance script gates what is built and is **not** a claim of completeness. Superseded status line follows. |
| **Superseded roadmap status** | **Rows 1–13 are complete.** **Row 14, the remaining functional UI parity gaps, the real-world validation run, the acceptance expansion and the final clean-checkout audit are not started.** Row 14's plan was corrected on 2026-08-08 (`d9120c8`) before implementation and its correction blocks overrule the surrounding text. **Remaining UI parity gaps** (`docs/PRODUCT-CAPABILITY-MATRIX.md` §9): impact has no UI view · selector alternatives are not rendered · trace/test-observed-call evidence has no dedicated surface · `check`'s trust verdict is CLI-only. **Closed this session:** narrow-width QA (mechanism + all history and contract views), keyboard navigation (found and fixed a 1.81:1 focus ring), and the corrupt-history UI. The acceptance script gates what is built and is **not** a claim of completeness. |
...
- **Delegation is HARD-DEAD as of 2026-08-10 evening: the organisation's *monthly* spend limit.**
  This supersedes the weekly-limit note below it, which was **refuted by measurement** — a cheap
  read-only probe succeeded on 2026-08-10 ~16:15, well before the recorded Aug 12 reset, so
  delegation was available again and Slice 14b-i *was* implemented by a subagent. That agent was
  then killed twice: once silently mid-slice (recovered by resuming it with `SendMessage`, which
  worked and is worth trying first), and once terminally by the monthly limit, with
  *"You've hit your org's monthly spend limit"*. **Until the limit is raised or the month rolls
  over, everything is orchestrator-direct.** Seven agents have now been killed by limits on this
  project and the recorded fallback — inspect, preserve valid work, verify in the orchestrator —
  has applied every time without losing work.
- **This machine SIGKILLs heavy `cargo` runs.** Several `cargo test --workspace` runs died with
  exit 137 (SIGKILL) or 144, and one full-gate run was **contaminated** by a subagent editing
  source mid-run and had to be discarded rather than reported. Two consequences worth not
  rediscovering: **never run the gate while an implementation agent is working** (they contend for
  the build lock — a `cargo test` blocked 4m12s at 4% CPU waiting for it), and treat a target that
  aborts with **no `test result` line** as an environment kill until it is reproduced in isolation.
  `nerve-store --test prune` did exactly that and passes 6/6 alone and in the clean re-run.
- **Superseded delegation note — delegation hit a *weekly* limit on 2026-08-10, resetting Aug 12 07:00 America/Los_Angeles**,
